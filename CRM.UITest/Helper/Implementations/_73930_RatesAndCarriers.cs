...
{
    public class _73930_RatesAndCarriers : IRatesAndCarriers
    {
        ...
        private static IAssignIsGuaranteedCarrierPriceToGuaranteedCarrierAgent _assignIsGuaranteedCarrierPriceToGuaranteedCarrierAgent;
        public _73930_RatesAndCarriers(IGetRateResultsandCarriers getRateResultsandCarriers, IAddServiceDays addServiceDays, IRateCharges rateCharges,
           ...
           IAssignIsGuaranteedCarrierPriceToGuaranteedCarrierAgent assignIsGuaranteedCarrierPriceToGuaranteedCarrierAgent)
        {
            ...
            _assignIsGuaranteedCarrierPriceToGuaranteedCarrierAgent = assignIsGuaranteedCarrierPriceToGuaranteedCarrierAgent;
        }
           ...
           out string errorMessage)
        {
            ...
            XElement responseXml = _getRateResultsandCarriers.GetRateResultsandCarrier(rateRequestXml,
                rateRequest.RatingLevel, callProxyApiVersion2, out errorMessage, true);
            ...
            out string errorMessage)
        {
            ...
            XElement responseXml = _getRateResultsandCarriers.GetRateResultsandCarrier(
                rateRequestXml, rateRequest.RatingLevel, callProxyApiVersion2, out errorMessage, true);
            ...
            for (int i = 0; i < listPriceSheets.Count(); i++)
            {
                ...
                foreach (XElement charge in listPriceSheets[i].Elements("AssociatedCarrierPricesheet").Elements("PriceSheet").Elements("Charges").Elements("Charge"))
                {
                    ...
                    {
                        fuelAmountValue = charge.Elements("Rate").FirstOrDefault().Value;
                    }
                }
        ...
        private static IEnumerable<RateResultCarrierViewModel> BuildRateResultCarrierViewModel(XElement responseXml, double shipmentValue, RateConstraintsModel constraints, List<RateItemModel> items, IGetMaximumLiability getMaximumLiability, string customerName)
        {
            ...
            {
                carriersandRates = _assignIsGuaranteedCarrierPriceToGuaranteedCarrierAgent.AssignIsGuaranteedCarrierPrice(carriersandRates);
            }
        ...
        private static RateResultCarrierViewModel AddRateResultCarrierViewModel(XElement priceSheetElement, decimal shipmentCoverageValue, List<CarrierRateViewModel> carriers, List<RateItemModel> items, IGetMaximumLiability getMaximumLiability, bool isDefaultInsAll)
        {
            ...
            if(carriers.Any(c => c.CarrierCode.Equals(rateResultCarrierViewModel.ScacCode)))
            {
        ...
        private static List<RateResultCarrierViewModel> AddGuaranteedCarriers(List<RateResultCarrierViewModel> rateResultCarrierViewModel, List<CarrierRateViewModel> carriers, decimal shipmentCoverageValue)
        {
            ...
            {
                if(carriers.Any(c => c.CarrierCode.Equals(rateResult.ScacCode)))
                {
                    ...
                    guaranteedCharges = new List<CostComponents>
                    {
                        new CostComponents()
                        {
                            ...
                            FuelPercentage = rateResult.Charges[0].FuelPercentage
                        }
                    };
                    ...
                    foreach (RateResultCarrierViewModel rate in guaranteedRate)
                    {
                        ...
                        {
                            if(rate.ScacCode == guaranteedCarrier.CarrierCode)
                            {
        ...
        private static void GetGuranteedStandardRates(RateResultCarrierViewModel insuredRateCharges, double percentage, double minCharge, XElement priceSheetElement)
        {
            ...
            {
                decimal guaranteeRate = 0;
                if (insuredRateCharges.ScacCode == "FXFE" || insuredRateCharges.ScacCode == "FXNL" || insuredRateCharges.ScacCode == "FXFC")
                {
                    guaranteeRate = (Convert.ToDecimal(percentage) / 100) * rates.LineHaul;
                    guaranteeRate = guaranteeRate >= Convert.ToDecimal(minCharge) ? guaranteeRate : Convert.ToDecimal(minCharge);
                    decimal fuelSurcharge = (rates.FuelPercentage / 100) * (rates.LineHaul + guaranteeRate);
                    decimal totalGuaranteeRate = fuelSurcharge + Math.Round(Convert.ToDecimal(guaranteeRate), 2) + rates.LineHaul;
                    rates.FuelCost = fuelSurcharge;
                    rates.Assessorial += Math.Round(Convert.ToDecimal(guaranteeRate), 2);
                    rates.TotalCost = rates.LineHaul + rates.Assessorial + fuelSurcharge;
                }
                else
                {
                    guaranteeRate = (Convert.ToDecimal(percentage) / 100) * rates.TotalCost;
                    guaranteeRate = guaranteeRate >= Convert.ToDecimal(minCharge) ? guaranteeRate : Convert.ToDecimal(minCharge);
            ...
            if (!insuredRateCharges.IsInsuredRates)
                {
                    ...
                    priceSheetElement.Element("Charges").Add(CreateGuaranteedRateChargeNode(Convert.ToDouble(guaranteeRate), sequenceNumber));
                }
            }
        }
