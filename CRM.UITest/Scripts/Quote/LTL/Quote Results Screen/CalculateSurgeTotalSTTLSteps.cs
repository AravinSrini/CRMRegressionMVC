...
{
    [Binding]
    public class CalculateSurgeTotal_STTLSteps : ObjectRepository
    {
        ...
        [When(@"I am on the quote results page to calculate STTL (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*) and (.*)")]
        public void WhenIAmOnTheQuoteResultsPageToCalculateSTTLAnd(string Username, string customerAcc, string userType, string oZip,
      ...
      //      BaseMarkupBreakDownCalcuation step2 = new BaseMarkupBreakDownCalcuation();
        ...
        [Then(@"Surge total value will be calculated (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*), (.*) and (.*)")]
        public decimal ThenSurgeTotalValueWillBeCalculated(string Username, string customerAcc, string userType, string oZip,
            string oCity, string oState,
            string dZip, string dCity, string dState,
            string classification, string quantity, string weight, string oServices, string dServices)
        {
        ...
        [Then(@"The calculated STTL value should match with the UI values in quote results page")]
        public void ThenTheCalculatedSTTLValueShouldMatchWithTheUIValuesInQuoteResultsPage()
        {
            Report.WriteLine("Calculated Surge total should match with the UI");
            string UITotalSurge = Gettext(attributeName_xpath, ".//*[@id='Grid-Rate-List-Large-NonGuranteed']/tbody/tr[1]/td[4]/ul[1]/li[1]");
            Report.WriteLine("Matching displayed Bump Linehaul value with UI");
            string FinalSTTL = "* $" + Math.Round(STTL, 2);
            Assert.AreEqual(UITotalSurge, FinalSTTL);
        }
    }
}
