@Sprint59 @17437 
Feature: InternationalTileOnShipmentServiceScreenInRateRequestProcess_Desktop
...
Scenario Outline: Verify that logged in user has access to the Quotes
	...
	Examples: 
	| Scenario | Username | Password | UserDropdown    | QuotespageTitle | QuotespageSubtitle                                         | SubmitRateRequestBtn |
	| S1       | zzzz     | Te$t1234 | TESTING TESTING | Quote List      | Submitted rate requests within the past 30 days are shown. | Submit Rate Request  |
...
Scenario Outline: Verify that user can able to see the International Type model when click on the International Tile
	...
	And I should see the text '<Quotestext>', '<BacktoQuoteList>' button in the Quotes landing page
	...
	Examples: 
	| Scenario | Username | Password | UserDropdown | Quotestext | BacktoQuoteList    | International | InternationalTypetext |
	| S1       | zzzz     | Te$t1234 | ZZZZ TEST    | Get Quote  | Back to Quote List | International | International Type    |
...
Scenario Outline: Verify that user can able to see Type Drop down and can able to select an option from Type drop down in the  International Type model
	...
	Examples: 
	| Scenario | Username | Password | UserDropdown | Quotestext | BacktoQuoteList    | defaulttext    | 
	| S1       | zzzz     | Te$t1234 | ZZZZ TEST    | Get Quote  | Back to Quote List | Select Type... | 
...
Scenario Outline: Verify that user can able to see Level Drop down and can able to select an option from Level drop down in the International Type model
	...
	Examples: 
	| Scenario | Username | Password | UserDropdown | GetQuotetext | BacktoQuoteListBtn | defaulttext     | 
	| S1       | zzzz	  | Te$t1234 | ZZZZ TEST    | Get Quote    | Back to Quote List | Select Level... | 
...
Scenario Outline: Verify that user can able to see the Continue button and can able to click on the Continue button in the International Type model
	...
	Examples: 
	| Scenario | Username | Password | UserDropdown | GetQuotetext | BacktoQuoteListBtn | defaulttext  |
	| S1       | zzzz     | Te$t1234 | ZZZZ TEST    | Get Quote    | Back to Quote List | Continue     |
...
Scenario Outline: Verify that user can able to see the Cancel button and International Type model should close by clicking on the Cancel button in the International Type model
	...
	Examples: 
	| Scenario | Username | Password | UserDropdown | GetQuotetext | BacktoQuoteListBtn | defaulttext | InternationalTypetext | Quotestext | BacktoQuoteList    |
	| S1       | zzzz     | Te$t1234 | ZZZZ TEST    | Get Quote    | Back to Quote List | Close       | International Type    | Get Quote  | Back to Quote List |
...
Scenario Outline: Verify that user can able to navigate to the existing International shipping information screen when click the continue button with selection of valid Type & Level
	...
	| S1       | zzzz     | Te$t1234 | ZZZZ TEST    | Get Quote    | Back to Quote List | Rate Request       | Follow the steps below to receive a rate. | Shipment Information | Back to Quote List | International |
...
Scenario Outline: Verify that user can able to see the error message when click on Continue butoon without selecting any options in the International Type model
	...
	Examples: 
	| Scenario | Username | Password | UserDropdown | GetQuotetext | BacktoQuoteListBtn | defaulttext  | Errormessage                          |
	| S1       | zzzz     | Te$t1234 | ZZZZ TEST    | Get Quote    | Back to Quote List | Continue     | PLEASE ENTER ALL REQUIRED INFORMATION |