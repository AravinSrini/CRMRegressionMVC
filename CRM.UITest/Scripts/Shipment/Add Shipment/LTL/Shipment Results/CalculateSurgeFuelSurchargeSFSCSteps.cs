...
{
    [Binding]
    public class CalculateSurgeFuelSurchargeSFSCSteps : ObjectRepository
    {
        ...
        [Then(@"SFSC value will be calculated (.*)")]
        public decimal ThenSFSCValueWillBeCalculated(string CustomerName)
        {
            Report.WriteLine("Calculating Surge Fuel Surcharge");
            ...
            rlist = GetRatesAndCarriersAPICall.BuildIndividualAccessorialModelFromCarrierPriceSheet(CustomerName, "service", _ShipmentInformationModel.oCity,
                _ShipmentInformationModel.oZip, _ShipmentInformationModel.oState, "USA", _ShipmentInformationModel.dCity,
                _ShipmentInformationModel.dZip, _ShipmentInformationModel.dState, "USA", _ShipmentInformationModel.oServices,
                _ShipmentInformationModel.dServices, _ShipmentInformationModel.classification, Convert.ToDouble(_ShipmentInformationModel.quantity), "skids",
        ...
        [Then(@"The calculated Surge Fuel Surcharge value should match with the UI value")]
        public void ThenTheCalculatedSurgeFuelSurchargeValueShouldMatchWithTheUIValue()
        {
            Report.WriteLine("Calculated Surge Fuel Surcharge Value should match with the UI");
            string UIFuelSurchargeShip = Gettext(attributeName_xpath, ".//*[@id='ShipmentResultTable']/tbody/tr[1]/td[4]/div[2]");
            Report.WriteLine("Matching displayed Surge Fuel Surcharge value with UI");
            string FinalSFSC = "Fuel: $" + Math.Round(SFSC, 2);
            Assert.AreEqual(UIFuelSurchargeShip, FinalSFSC);
        }
